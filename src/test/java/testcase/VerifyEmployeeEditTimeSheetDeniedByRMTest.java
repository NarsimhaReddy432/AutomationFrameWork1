...
public class VerifyEmployeeEditTimeSheetDeniedByRMTest extends ExtentReportClass {
    ...
    public void testEmployeeEditTimeSheetDeniedByRM() {
        ...
        try {
            ...
            if (driver.getTitle().contains("Time Keeper")) {
                ...
                if (objLogin.getTextMyTimeMenu().equalsIgnoreCase("My Time")) {
                    ...
                    if(objLogin.employeeEditTimeSheetStatusDeniedByRM()){
                        flag=true;
                    }
                    Assert.assertTrue(true);
                    screenShotPath = objCaptureScreenshot.captureScreenshot(screenshotName);
                    logger.log(LogStatus.PASS, "Verification of editing timesheet by employee, once it is denied by RM, is successful" + logger.addScreenCapture(screenShotPath));
                }
            }
        } catch (Exception e) {
            logger.log(LogStatus.ERROR, e.getMessage());
            logger.log(LogStatus.ERROR, "Exception while Verifying of editing timesheet by employee, once it is denied by RM" + logger.addScreenCapture(screenShotPath));
        }
    }
